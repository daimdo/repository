...
while not file_found:
    user_read_create = input(
        "Read from an existing .txt file (R) or create a new .txt file (C): "
    )
    ...
    if user_read_create.upper() == "R":
        ...
        while user_numb_chosen < 1 or user_numb_chosen > len(file_list):
            user_numb_chosen = int(
                input(f"Out of range. Enter a number between 1 and {len(file_list)}: ")
            )
    ...
    # Create new ".txt" file in user directory. Check if filename ends with .txt and existence in user directory
        ...
        while True:
            ...
            if not filename.endswith(".txt"):
                print(
                    "Please type a filename that ends with '.txt' (E.g. 'example.txt')"
                )
                continue
            if os.path.exists(filename):
                print(f"File '{filename}' already exists, use another filename.")
                file_overwrite = input(
                    "Do you want to overwrite the file? Yes (Y) or No (N)? "
                )
        ...
        while True:
            new_text = input(
                f"Type text to line {line_num} or don't and press Enter to finish: "
            )
            if new_text == "":
                break
            text += new_text + "\n"
        ...
        if occurrences == 1:
            ...
            if replace_all.upper() == "Y":
                ...
                if line_num > 0 and line_num <= len(lines):
                    ...
                    occurrence_num = int(
                        input(
                            f"Which occurrence to replace? (1-based index, total {occurrences}): "
                        )
                    )
                    ...
                    print(text)
                    print(
                        f"Replaced {occurrences} occurrences of '{replacement_text}' in line {line_num}."
                    )
                    total_occurrences_replaced += occurrences
                else:
